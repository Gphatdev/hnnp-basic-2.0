module.exports.config = {
  ...
  module.exports.run = async function ({ Currencies, api, event, args, Users,permssion }) {
    ...
    if (amountOperations.includes(args[0])) {
      const amount = Number(mon);
      if (!Number.isFinite(amount) || ((args[0] === "/" || args[0] === "√") && amount === 0)) {
        return api.sendMessage("Số tiền hoặc giá trị nhập vào không hợp lệ", event.threadID);
      }
    }
    try { switch (args[0]) {
    case "+": {
    ...
    const bet = args[1] === 'all' ? Number(money2) : Number(args[1]);
    if (!Number.isFinite(bet) || bet <= 0 || bet > Number(money2)) return api.sendMessage({body:"Số tiền chuyển không hợp lệ hoặc vượt quá số dư của bạn",attachment: await i(link)},event.threadID)
