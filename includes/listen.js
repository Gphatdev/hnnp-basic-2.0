module.exports = function({
  ...
  const logIncomingMessage = (event, threadName) => {
    if (!['message', 'message_reply'].includes(event.type) || String(event.senderID) === botUserID) return;
    const senderID = String(event.senderID || 'unknown');
    const senderName = global.data.userName.get(senderID) || senderID;
    const body = typeof event.body === 'string' && event.body.length > 0 ? event.body : '[tin nhắn không có nội dung]';
    const replyMark = event.type === 'message_reply' ? ' (reply)' : '';
    console.log(`[ MESSAGE ] [${new Date().toLocaleTimeString('vi-VN')}] ${senderName} <${senderID}> | ${threadName || event.threadID}${replyMark}: ${body}`);
  };
  ...
  return async function(event) {
    logIncomingMessage(event, global.data.threadInfo.get(String(event.threadID))?.threadName);
